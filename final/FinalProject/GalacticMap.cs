...
class GalacticMap: GameForm
{
    ...
    public GalacticMap() : base()
    {
        ...
        // 780, 1260, 
        StarSystem bilbringi = new StarSystem("", 4, 780, 1260, "Bilbringi is synonymous with shipbuilding and logistics, its shipyards a vital cog in the Galactic Empire's war machine. The system has seen many battles, notably during the Yuuzhan Vong War, where control of its shipyards was fiercely contested. Today, Bilbringi remains a key hub of industrial might and strategic relevance. Its legacy continues to be one of pivotal importance in galactic conflicts.");
        // 1750, 510, 
    ...
    public void DisplayGalacticMap()
    {
        GameForm gameForm = new GameForm();
        foreach (StarSystem starsystem in starsystems)
        {
            gameForm.AddPicture(planetImageLocation, (int)starsystem.getPositionX()/3, (int)starsystem.getPositionY()/5, 40);
        }
        // Pen pen = new Pen(Color.FromArgb(255, 0, 0, 0));
        // e.Graphics.DrawLine(pen, 20, 10, 300, 100);

        // foreach (List<StarSystem> hyperlane in hyperlanes)
        // {
        //     planetImageLocation
        // }
        gameForm.ShowDialog();
    ...
    {
        List<StarSystem> hyperlane = new List<StarSystem>{system1, system2};
